...
![llama.cpp](../_assets/llama-cpp-settings.png)
...
![llama.cpp Import](../_assets/llama-cpp-import.png)
...
### Intel Arc with OpenVINO (source builds)

Install the Intel GPU driver, OpenVINO Runtime, and OpenCL development
headers. From the Jan checkout, source OpenVINO's environment and build the
worker:

```bash
source /opt/intel/openvino/setupvars.sh
make build-engine JAN_ENGINE_VARIANT=openvino
```

The OpenVINO worker defaults to the GPU for this build. To override the
target, set the environment variable before launching Jan:

```bash
export GGML_OPENVINO_DEVICE=GPU
make dev JAN_ENGINE_VARIANT=openvino
```

Use `CPU` or `NPU` instead when those targets are intentional. The same
variable can be set in Jan's Llama.cpp environment settings for a packaged
build.

Use `GPU.0` or `GPU.1` when more than one Intel GPU is present. OpenVINO
runtime libraries are staged beside the worker; the Intel GPU driver remains
a system dependency.

### Intel Arc with SYCL (source builds)

SYCL uses Intel oneAPI and Level Zero directly. Source the oneAPI environment,
then build the separate SYCL worker variant:

```bash
source /opt/intel/oneapi/setvars.sh
make build-engine JAN_ENGINE_VARIANT=sycl JAN_ENGINE_JOBS=10
```

For Arc GPUs, select the Intel GPU through the SYCL runtime when launching
Jan:

```bash
export ONEAPI_DEVICE_SELECTOR=level_zero:gpu
make dev JAN_ENGINE_VARIANT=sycl
```

SYCL and OpenVINO are alternative engine variants; they are not a hybrid
execution path. SYCL packages include the oneAPI runtime libraries, while the
Intel GPU driver and Level Zero implementation remain system dependencies.

To package both backends in one worker, source both toolchains and use the
combined variant. Jan's Hardware settings can then select the enumerated
OpenVINO or SYCL device for each model:

```bash
source /opt/intel/openvino/setupvars.sh
source /opt/intel/oneapi/setvars.sh
make build-engine JAN_ENGINE_VARIANT=openvino-sycl JAN_ENGINE_JOBS=10
```

For the combined worker, set `GGML_OPENVINO_DEVICE=GPU` and
`ONEAPI_DEVICE_SELECTOR=level_zero:gpu` in the Llama.cpp environment field
when using the corresponding backend.
...
**CUDA 12.0 (recommended):**
- `linux-avx2-cuda-cu12.0-x64` (most common)
- `linux-avx512-cuda-cu12.0-x64` (newer Intel/AMD CPUs)
- `linux-avx-cuda-cu12.0-x64` (older CPUs)
- `linux-cuda-12-common_cpus-x64` (broadest CPU compatibility)

**CUDA 11.7:**
- `linux-avx2-cuda-cu11.7-x64` (most common)
- `linux-avx512-cuda-cu11.7-x64` (newer Intel/AMD CPUs)
- `linux-avx-cuda-cu11.7-x64` (older CPUs)
- `linux-cuda-11-common_cpus-x64` (broadest CPU compatibility)

**CUDA 13:**
- `linux-cuda-13-common_cpus-x64`

### CPU Only
- `linux-avx2-x64` (most modern CPUs, recommended)
- `linux-avx512-x64` (newer Intel/AMD CPUs with AVX-512)
- `linux-avx-x64` (older CPUs)
- `linux-noavx-x64` (very old CPUs)
- `linux-common_cpus-x64` (broadest compatibility)

### Other Graphics Cards (Vulkan — AMD, Intel Arc)
- `linux-vulkan-x64` (recommended for GPU acceleration)
- `linux-vulkan-common_cpus-x64` (Vulkan with broader CPU support)

<Callout type="info">
**Quick Test**: Start with `linux-avx2-cuda-cu12.0-x64` for NVIDIA, `linux-vulkan-x64` for AMD/Intel Arc, or `linux-avx2-x64` for CPU-only.
</Callout>
...
- `macos-arm64` (Apple Silicon — M1/M2/M3/M4, recommended)
...
3. Increase Batch Size (per-model) and uBatch Size (engine)
4. Close other applications using memory

**If running out of memory:**
1. Set **Max Concurrent Models** to 1 so the router unloads the previous model before loading a new one
